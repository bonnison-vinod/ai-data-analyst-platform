...
{{
  ...
  "summary": "one sentence summary"
    ...
    def generate_smart_prompt(self, metadata: Dict[str, Any], user_query: str) -> str:
        ...
        prompt = f"""Data Brain: Recommend analysis for user query.

META: {json.dumps(relevant_meta, separators=(',', ':'))}
Q: "{user_query}"
HINT: likely_{likely_analysis}

TYPES: ts=time_series, corr=correlation, piv=pivot, dist=distribution, comp=comparison, out=outlier, stat=statistical
CHARTS: L=line, B=bar, P=pie, S=scatter, H=histogram, X=box, M=heatmap
